...
def main(_):
  print (FLAGS.checkpoint_path, FLAGS.model)
  # Build the inference graph.
  g = tf.Graph()
  with g.as_default():
    model = inference_wrapper.InferenceWrapper(model_name=FLAGS.model)
  ...
  tf.logging.info("\nRunning caption generation on %d\n", len(filenames))  
  ...
  with tf.Session(graph=g, config=config) as sess:
    ...
    for filename in filenames:
      ...
      _ += 1
      if _%100 == 0: print ( FLAGS.tmpfile, _, '%2.1f' % (_/len(filenames)*100) )
